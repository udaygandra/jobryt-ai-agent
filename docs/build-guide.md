...
In n8n: **Settings → Credentials → Add Credential** for each one. One correction to flag before you set these up: **Adzuna does not accept a header-based API key** — its official docs specify `app_id` and `app_key` as two separate URL query parameters. n8n's generic "Query Auth" credential type only holds one name/value pair, so a single stored credential can't carry both. Details on the actual working setup for each service below.

### Adzuna — query parameters via environment variables (not a stored credential)

Because Adzuna needs two values and n8n's built-in Query Auth credential only supports one, the cleanest approach is to **not** use the Credentials panel for Adzuna at all. Instead:

1. Add both values as environment variables where n8n runs:
   - **Docker**: add to your `docker-compose.yml` under the `n8n` service:
     ```yaml
     environment:
       - ADZUNA_APP_ID=your_id_here
       - ADZUNA_APP_KEY=your_key_here
     ```
     then `docker compose up -d` again to pick them up.
   - **`npx n8n`**: export them in your shell before starting n8n: `export ADZUNA_APP_ID=... ADZUNA_APP_KEY=...` then `npx n8n`.
2. In the **HTTP Request** node (the Adzuna Fetch node), open **Query Parameters**, toggle it on, and add two parameters:
   - `app_id` → value: `={{ $env.ADZUNA_APP_ID }}`
   - `app_key` → value: `={{ $env.ADZUNA_APP_KEY }}`
3. This keeps the actual secret out of the workflow JSON (only the env-variable *name* is stored, not the value) — so it's still safe to export/share the workflow later.

### Gemini — Header Auth credential

1. **Credentials → Add Credential → Header Auth.**
2. Name: `x-goog-api-key`
3. Value: your key from aistudio.google.com.
4. Save it as something identifiable like "Gemini Header Auth."
5. On each Gemini HTTP Request node: **Authentication → Generic Credential Type → Header Auth**, then select this credential from the dropdown.

### Hunter.io — Header Auth credential

Hunter's docs confirm it accepts the key either as a query parameter or as a header — the header route is cleaner in n8n since it's one name/value pair, same pattern as Gemini:

1. **Credentials → Add Credential → Header Auth.**
2. Name: `X-API-KEY`
3. Value: your key from hunter.io/api-keys.
4. Attach it the same way on the Hunter HTTP Request node (**Authentication → Generic Credential Type → Header Auth**).

### Telegram — Telegram API credential

1. In Telegram, message `@BotFather` → `/newbot` → follow the prompts (choose a name and a unique username ending in `bot`) → it returns a token like `123456789:AAExampleToken`.
2. In n8n: **Credentials → Add Credential → search "Telegram API."**
3. Paste the token into the **Access Token** field. That's the only field needed.
4. To find your own numeric chat ID (so the bot knows where to send approval messages), send any message to your new bot, then visit `https://api.telegram.org/bot<YOUR_TOKEN>/getUpdates` in a browser — your chat ID appears in the JSON response under `message.chat.id`.

### Gmail / SMTP — App Password (simplest for self-hosted)

n8n's native Gmail node uses OAuth2, which requires setting up a Google Cloud project and an OAuth consent screen — more setup than it's worth for a personal tool. Simpler route: use the **Send Email** node with SMTP + a Gmail **App Password**.

1. Your Google account needs 2-Step Verification turned on first (myaccount.google.com/security).
2. Go to myaccount.google.com/apppasswords, create a new app password (name it "n8n"), and copy the 16-character code shown.
3. In n8n: **Credentials → Add Credential → SMTP.**
   - Host: `smtp.gmail.com`
   - Port: `465` (SSL) or `587` (TLS)
   - User: your full Gmail address
   - Password: the 16-character app password (not your regular Google password)
4. Use this credential on a **Send Email** node wherever the workflow needs to actually send something.
...
- [ ] The score gate only lets ≥80 jobs through
